...
define(['./defined-a5305fd6', './Cartesian2-b941a975', './Transforms-e81b498a', './Cartesian3-5587e0cf', './Check-0f680516', './ComponentDatatype-4ab1a86a', './GeometryAttribute-a065274b', './GeometryAttributes-ae79d5fa', './GeometryInstance-2afc3d77', './GeometryOffsetAttribute-102da468', './GeometryPipeline-8667588a', './IndexDatatype-be8e0e62', './Math-79d70b44', './Matrix2-81068516', './PolygonPipeline-ddb4fc8b', './RectangleGeometryLibrary-978c6f49', './VertexFormat-26a1b05a', './GeographicProjection-bcd5d069', './Resource-0c25a925', './_commonjsHelpers-89c9b271', './combine-eceb7722', './defer-bfc6471e', './RuntimeError-8d8b6ef6', './WebGLConstants-d81b330d', './AttributeCompression-59630bdd', './EncodedCartesian3-3b2d3f49', './IntersectionTests-8d40a746', './Plane-20e816c1', './EllipsoidRhumbLine-90229f69'], (function (defined, Cartesian2, Transforms, Cartesian3, Check, ComponentDatatype, GeometryAttribute, GeometryAttributes, GeometryInstance, GeometryOffsetAttribute, GeometryPipeline, IndexDatatype, Math$1, Matrix2, PolygonPipeline, RectangleGeometryLibrary, VertexFormat, GeographicProjection, Resource, _commonjsHelpers, combine, defer, RuntimeError, WebGLConstants, AttributeCompression, EncodedCartesian3, IntersectionTests, Plane, EllipsoidRhumbLine) { 'use strict';

  const positionScratch = new Cartesian3.Cartesian3();
  const normalScratch = new Cartesian3.Cartesian3();
  const tangentScratch = new Cartesian3.Cartesian3();
  const bitangentScratch = new Cartesian3.Cartesian3();
  ...
  function calculateAttributes(
    ...
    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
      for (let i = 0; i < length; i += 3) {
        const p = Cartesian3.Cartesian3.fromArray(positions, i, positionScratch);
  ...
  const v1Scratch = new Cartesian3.Cartesian3();
  const v2Scratch = new Cartesian3.Cartesian3();
  ...
  function calculateAttributesWall(positions, vertexFormat, ellipsoid) {
    ...
    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
      for (let i = 0; i < length; i += 6) {
        const p = Cartesian3.Cartesian3.fromArray(positions, i, positionScratch);
        const p1 = Cartesian3.Cartesian3.fromArray(positions, (i + 6) % length, v1Scratch);
        if (recomputeNormal) {
          const p2 = Cartesian3.Cartesian3.fromArray(positions, (i + 3) % length, v2Scratch);
        ...
        if (Cartesian3.Cartesian3.equalsEpsilon(p1, p, Math$1.CesiumMath.EPSILON10)) {
  ...
  function constructExtrudedRectangle(rectangleGeometry, computedOptions) {
    ...
    let offsetValue;
    const hasOffsets = defined.defined(offsetAttributeValue);
    if (hasOffsets) {
    ...
    for (i = 0; i < length - 1; i += 2) {
      ...
      const p1 = Cartesian3.Cartesian3.fromArray(wallPositions, upperLeft * 3, v1Scratch);
      const p2 = Cartesian3.Cartesian3.fromArray(wallPositions, upperRight * 3, v2Scratch);
      if (Cartesian3.Cartesian3.equalsEpsilon(p1, p2, Math$1.CesiumMath.EPSILON10)) {
        continue;
      }
  ...
  const stNwScratch = new Cartesian2.Cartographic();
  ...
  const centerScratch = new Cartesian2.Cartographic();
  /**
  ...
  RectangleGeometry.createGeometry = function (rectangleGeometry) {
    ...
    if (stRotation !== 0 || rotation !== 0) {
      const center = Cartesian2.Rectangle.center(rectangle, centerScratch);
  ...
  const unrotatedTextureRectangleScratch = new Cartesian2.Rectangle();
  const points2DScratch = [new Cartesian2.Cartesian2(), new Cartesian2.Cartesian2(), new Cartesian2.Cartesian2()];
  const rotation2DScratch = new Matrix2.Matrix2();
  const rectangleCenterScratch = new Cartesian2.Cartographic();
  ...
  function textureCoordinateRotationPoints(rectangleGeometry) {
    ...
    const rectangle = Cartesian2.Rectangle.clone(
    ...
    const boundingRectangleCenter = Cartesian2.Rectangle.center(
      boundingRectangle,

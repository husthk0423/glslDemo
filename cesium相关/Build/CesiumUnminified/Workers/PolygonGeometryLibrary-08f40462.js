...
define(['exports', './ArcType-b714639b', './arrayRemoveDuplicates-1c85c3e7', './Cartesian2-b941a975', './Cartesian3-5587e0cf', './ComponentDatatype-4ab1a86a', './defined-a5305fd6', './EllipsoidRhumbLine-90229f69', './GeometryAttribute-a065274b', './GeometryAttributes-ae79d5fa', './GeometryPipeline-8667588a', './IndexDatatype-be8e0e62', './Math-79d70b44', './Matrix2-81068516', './PolygonPipeline-ddb4fc8b', './Transforms-e81b498a'], (function (exports, ArcType, arrayRemoveDuplicates, Cartesian2, Cartesian3, ComponentDatatype, defined, EllipsoidRhumbLine, GeometryAttribute, GeometryAttributes, GeometryPipeline, IndexDatatype, Math$1, Matrix2, PolygonPipeline, Transforms) { 'use strict';
  ...
  const distance2DScratch = new Cartesian2.Cartesian2();
  function getPointAtDistance2D(p0, p1, distance, length) {
    Cartesian2.Cartesian2.subtract(p1, p0, distance2DScratch);
    Cartesian2.Cartesian2.multiplyByScalar(
      distance2DScratch,
      distance / length,
      distance2DScratch
    );
    Cartesian2.Cartesian2.add(p0, distance2DScratch, distance2DScratch);
    return [distance2DScratch.x, distance2DScratch.y];
  }

  const distanceScratch = new Cartesian3.Cartesian3();
  ...
  const scratchCartesian0 = new Cartesian3.Cartesian3();
  ...
  PolygonGeometryLibrary.subdivideTexcoordLine = function (
    ...
    const length2D = Cartesian2.Cartesian2.distance(t0, t1);
  ...
  PolygonGeometryLibrary.subdivideLine = function (p0, p1, minDistance, result) {
    ...
    const length = Cartesian3.Cartesian3.distance(p0, p1);
  ...
  const scaleToGeodeticHeightN1 = new Cartesian3.Cartesian3();
  const scaleToGeodeticHeightN2 = new Cartesian3.Cartesian3();
  const scaleToGeodeticHeightP1 = new Cartesian3.Cartesian3();
  const scaleToGeodeticHeightP2 = new Cartesian3.Cartesian3();
  ...
  PolygonGeometryLibrary.scaleToGeodeticHeightExtruded = function (
    ...
    if (
      defined.defined(geometry) &&
      defined.defined(geometry.attributes) &&
  ...
  PolygonGeometryLibrary.polygonsFromHierarchy = function (
    ...
    while (queue.length !== 0) {
      ...
      const numChildren = defined.defined(holes) ? holes.length : 0;
      const polygonHoles = [];
      let j;
  ...
  const computeBoundingRectangleCartesian2 = new Cartesian2.Cartesian2();
  const computeBoundingRectangleCartesian3 = new Cartesian3.Cartesian3();
  ...
  PolygonGeometryLibrary.computeBoundingRectangle = function (
    ...
    for (let i = 0; i < length; ++i) {
      const p = Cartesian3.Cartesian3.clone(
        positions[i],
  ...
  PolygonGeometryLibrary.createGeometryFromPositions = function (
    ...
    const hasTexcoords = defined.defined(textureCoordinates);
    ...
    if (perPositionHeight) {
      ...
      if (hasTexcoords) {
        geometryOptions.attributes.st = new GeometryAttribute.GeometryAttribute({
          ...
          values: Cartesian2.Cartesian2.packArray(texcoords),
        });
      }
...
//# sourceMappingURL=PolygonGeometryLibrary-08f40462.js.map

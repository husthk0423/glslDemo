...
define(['exports', './Cartesian2-b941a975', './Cartesian3-5587e0cf', './Matrix2-81068516', './EllipsoidTangentPlane-fc899479', './Math-79d70b44', './PolylinePipeline-e6166610', './Transforms-e81b498a', './defined-a5305fd6', './Check-0f680516'], (function (exports, Cartesian2, Cartesian3, Matrix2, EllipsoidTangentPlane, Math$1, PolylinePipeline, Transforms, defined, Check) { 'use strict';
  ...
  function oneTimeWarning(identifier, message) {
    ...
    if (!defined.defined(warnings[identifier])) {
      ...
      console.warn(defined.defaultValue(message, identifier));
    }
  }
  ...
  const scratch2Array = [new Cartesian3.Cartesian3(), new Cartesian3.Cartesian3()];
  ...
  const scratchCartesian7 = new Cartesian3.Cartesian3();
  const scratchCartesian8 = new Cartesian3.Cartesian3();
  const scratchCartesian9 = new Cartesian3.Cartesian3();
  ...
  let cartographic = new Cartesian2.Cartographic();
  ...
  function subdivideHeights(points, h0, h1, granularity) {
    const p0 = points[0];
    const p1 = points[1];
    const angleBetween = Cartesian3.Cartesian3.angleBetween(p0, p1);
  ...
  const nextScratch = new Cartesian3.Cartesian3();
  const prevScratch = new Cartesian3.Cartesian3();
  ...
  function computeRotationAngle(start, end, position, ellipsoid) {
    ...
    const next = tangentPlane.projectPointOntoPlane(
      Cartesian3.Cartesian3.add(position, start, nextScratch),
      nextScratch
    );
    const prev = tangentPlane.projectPointOntoPlane(
      Cartesian3.Cartesian3.add(position, end, prevScratch),
      prevScratch
    );
    const angle = Cartesian2.Cartesian2.angleBetween(next, prev);

    return prev.x * next.y - prev.y * next.x >= 0.0 ? -angle : angle;
  }

  const negativeX = new Cartesian3.Cartesian3(-1, 0, 0);
  ...
  const westScratch = new Cartesian3.Cartesian3();
  const finalPosScratch = new Matrix2.Cartesian4();
  const heightCartesian = new Cartesian3.Cartesian3();
  ...
  function addPositions(
    ...
    for (let i = 0; i < centers.length; i += 3) {
      const center = Cartesian3.Cartesian3.fromArray(centers, i, centerScratch);
  ...
  const startPointScratch = new Cartesian3.Cartesian3();
  ...
  function computeRoundCorner(
    ...
    const angle = Cartesian3.Cartesian3.angleBetween(
      Cartesian3.Cartesian3.subtract(startPoint, pivot, scratch1),
      Cartesian3.Cartesian3.subtract(endPoint, pivot, scratch2)
    );
    const granularity =
    ...
    if (leftIsOutside) {
      m = Matrix2.Matrix3.fromQuaternion(
        Transforms.Quaternion.fromAxisAngle(
          Cartesian3.Cartesian3.negate(pivot, scratch1),
          angle / (granularity + 1),
          quaterion
        ),
        rotMatrix
  ...
  PolylineVolumeGeometryLibrary.angleIsGreaterThanPi = function (
    ...
    const tangentPlane = new EllipsoidTangentPlane.EllipsoidTangentPlane(position, ellipsoid);
    const next = tangentPlane.projectPointOntoPlane(
      Cartesian3.Cartesian3.add(position, forward, nextScratch),
      nextScratch
    );
    const prev = tangentPlane.projectPointOntoPlane(
      Cartesian3.Cartesian3.add(position, backward, prevScratch),
      prevScratch
    );
  ...
  PolylineVolumeGeometryLibrary.computePositions = function (
    ...
    for (let i = 1; i < length - 1; i++) {
      ...
      const forwardProjection = Cartesian3.Cartesian3.multiplyByScalar(
        surfaceNormal,
        Cartesian3.Cartesian3.dot(forward, surfaceNormal),
      ...
      const backwardProjection = Cartesian3.Cartesian3.multiplyByScalar(
        surfaceNormal,
        Cartesian3.Cartesian3.dot(backward, surfaceNormal),
      ...
      if (doCorner) {
        ...
        if (leftIsOutside) {
          pivot = Cartesian3.Cartesian3.add(
            position,
          ...
          start = Cartesian3.Cartesian3.add(
            pivot,
            Cartesian3.Cartesian3.multiplyByScalar(left, width, start),
            start
          );
          ...
          left = Cartesian3.Cartesian3.cross(surfaceNormal, forward, left);
          left = Cartesian3.Cartesian3.normalize(left, left);
          end = Cartesian3.Cartesian3.add(
            pivot,
            Cartesian3.Cartesian3.multiplyByScalar(left, width, end),
            end
          );
          if (
...
//# sourceMappingURL=PolylineVolumeGeometryLibrary-251081c9.js.map

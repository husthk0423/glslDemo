...
define(['./Cartesian2-b941a975', './AxisAlignedBoundingBox-1a14512c', './Transforms-e81b498a', './Cartesian3-5587e0cf', './defined-a5305fd6', './Check-0f680516', './TerrainEncoding-578da000', './Math-79d70b44', './Matrix2-81068516', './OrientedBoundingBox-c4615547', './WebMercatorProjection-7ce9285b', './RuntimeError-8d8b6ef6', './_commonjsHelpers-89c9b271', './createTaskProcessorWorker', './GeographicProjection-bcd5d069', './Resource-0c25a925', './combine-eceb7722', './defer-bfc6471e', './AttributeCompression-59630bdd', './ComponentDatatype-4ab1a86a', './WebGLConstants-d81b330d', './EllipsoidTangentPlane-fc899479', './IntersectionTests-8d40a746', './Plane-20e816c1'], (function (Cartesian2, AxisAlignedBoundingBox, Transforms, Cartesian3, defined, Check, TerrainEncoding, Math$1, Matrix2, OrientedBoundingBox, WebMercatorProjection, RuntimeError, _commonjsHelpers, createTaskProcessorWorker, GeographicProjection, Resource, combine, defer, AttributeCompression, ComponentDatatype, WebGLConstants, EllipsoidTangentPlane, IntersectionTests, Plane) { 'use strict';
  ...
  HeightmapTessellator.computeVertices = function (options) {
    ...
    const isGeographic = defined.defaultValue(options.isGeographic, true);
    ...
    const nativeRectangle = Cartesian2.Rectangle.clone(options.nativeRectangle);
    const rectangle = Cartesian2.Rectangle.clone(options.rectangle);
    ...
    const hasRelativeToCenter = defined.defined(relativeToCenter);
    relativeToCenter = hasRelativeToCenter ? relativeToCenter : Cartesian3.Cartesian3.ZERO;
    const includeWebMercatorT = defined.defaultValue(options.includeWebMercatorT, false);

    const exaggeration = defined.defaultValue(options.exaggeration, 1.0);
    const exaggerationRelativeHeight = defined.defaultValue(
    ...
    const structure = defined.defaultValue(
      options.structure,
      HeightmapTessellator.DEFAULT_STRUCTURE
    );
    const heightScale = defined.defaultValue(
      structure.heightScale,
      HeightmapTessellator.DEFAULT_STRUCTURE.heightScale
    );
    const heightOffset = defined.defaultValue(
      structure.heightOffset,
      HeightmapTessellator.DEFAULT_STRUCTURE.heightOffset
    );
    const elementsPerHeight = defined.defaultValue(
      structure.elementsPerHeight,
      HeightmapTessellator.DEFAULT_STRUCTURE.elementsPerHeight
    );
    const stride = defined.defaultValue(
      structure.stride,
      HeightmapTessellator.DEFAULT_STRUCTURE.stride
    );
    const elementMultiplier = defined.defaultValue(
      structure.elementMultiplier,
      HeightmapTessellator.DEFAULT_STRUCTURE.elementMultiplier
    );
    const isBigEndian = defined.defaultValue(
      structure.isBigEndian,
      HeightmapTessellator.DEFAULT_STRUCTURE.isBigEndian
    );

    let rectangleWidth = Cartesian2.Rectangle.computeWidth(nativeRectangle);
    let rectangleHeight = Cartesian2.Rectangle.computeHeight(nativeRectangle);
    ...
    for (let rowIndex = startRow; rowIndex < endRow; ++rowIndex) {
      ...
      for (let colIndex = startCol; colIndex < endCol; ++colIndex) {
        ...
        const position = new Cartesian3.Cartesian3();

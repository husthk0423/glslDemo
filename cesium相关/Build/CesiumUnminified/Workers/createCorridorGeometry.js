...
define(['./arrayRemoveDuplicates-1c85c3e7', './Transforms-e81b498a', './Cartesian3-5587e0cf', './Cartesian2-b941a975', './Check-0f680516', './ComponentDatatype-4ab1a86a', './PolylineVolumeGeometryLibrary-251081c9', './CorridorGeometryLibrary-d390924d', './defined-a5305fd6', './GeometryAttribute-a065274b', './GeometryAttributes-ae79d5fa', './GeometryOffsetAttribute-102da468', './IndexDatatype-be8e0e62', './Math-79d70b44', './PolygonPipeline-ddb4fc8b', './VertexFormat-26a1b05a', './GeographicProjection-bcd5d069', './Matrix2-81068516', './RuntimeError-8d8b6ef6', './Resource-0c25a925', './_commonjsHelpers-89c9b271', './combine-eceb7722', './defer-bfc6471e', './WebGLConstants-d81b330d', './EllipsoidTangentPlane-fc899479', './AxisAlignedBoundingBox-1a14512c', './IntersectionTests-8d40a746', './Plane-20e816c1', './PolylinePipeline-e6166610', './EllipsoidGeodesic-04edecba', './EllipsoidRhumbLine-90229f69'], (function (arrayRemoveDuplicates, Transforms, Cartesian3, Cartesian2, Check, ComponentDatatype, PolylineVolumeGeometryLibrary, CorridorGeometryLibrary, defined, GeometryAttribute, GeometryAttributes, GeometryOffsetAttribute, IndexDatatype, Math$1, PolygonPipeline, VertexFormat, GeographicProjection, Matrix2, RuntimeError, Resource, _commonjsHelpers, combine$1, defer, WebGLConstants, EllipsoidTangentPlane, AxisAlignedBoundingBox, IntersectionTests, Plane, PolylinePipeline, EllipsoidGeodesic, EllipsoidRhumbLine) { 'use strict';
  ...
  function addNormals(attr, normal, left, front, back, vertexFormat) {
    ...
    const forward = Cartesian3.Cartesian3.normalize(
      Cartesian3.Cartesian3.cross(left, normal, scratch1),
      scratch1
    );
  ...
  function combine(computedPositions, vertexFormat, ellipsoid) {
    ...
    for (i = 0; i < length; i += 3) {
      rightNormal = ellipsoid.geodeticSurfaceNormal(
        Cartesian3.Cartesian3.fromArray(rightEdge, i, scratch1),
        scratch1
      );
      leftNormal = ellipsoid.geodeticSurfaceNormal(
        Cartesian3.Cartesian3.fromArray(leftEdge, length - i, scratch2),
        scratch2
      );
      normal = Cartesian3.Cartesian3.normalize(
        Cartesian3.Cartesian3.add(rightNormal, leftNormal, normal),
        normal
      );
      addNormals(attr, normal, left, front, back, vertexFormat);
    ...
    rightNormal = ellipsoid.geodeticSurfaceNormal(
      Cartesian3.Cartesian3.fromArray(rightEdge, length, scratch1),
      scratch1
    );
    leftNormal = ellipsoid.geodeticSurfaceNormal(
      Cartesian3.Cartesian3.fromArray(leftEdge, length, scratch2),
      scratch2
    );
    normal = Cartesian3.Cartesian3.normalize(
      Cartesian3.Cartesian3.add(rightNormal, leftNormal, normal),
    ...
    for (i = 0; i < corners.length; i++) {
      ...
      if (defined.defined(l)) {
        ...
        for (j = 0; j < l.length / 3; j++) {
          ...
          previousPoint = Cartesian3.Cartesian3.fromArray(
            finalPositions,
            (start - j - 1) * 3,
            previousPoint
          );
          ...
          left = Cartesian3.Cartesian3.normalize(
            Cartesian3.Cartesian3.subtract(previousPoint, nextPoint, left),
            left
          );
          addNormals(attr, normal, left, undefined, back, vertexFormat);
          back -= 3;
        }
        outsidePoint = Cartesian3.Cartesian3.fromArray(
          finalPositions,
          pivot * 3,
          outsidePoint
        );
        previousPoint = Cartesian3.Cartesian3.subtract(
          Cartesian3.Cartesian3.fromArray(finalPositions, start * 3, previousPoint),
          outsidePoint,
          previousPoint
        );
        nextPoint = Cartesian3.Cartesian3.subtract(
          Cartesian3.Cartesian3.fromArray(finalPositions, (start - j) * 3, nextPoint),
          outsidePoint,
          nextPoint
        );
        left = Cartesian3.Cartesian3.normalize(
          Cartesian3.Cartesian3.add(previousPoint, nextPoint, left),
          left
        );
        ...
        for (j = 0; j < r.length / 3; j++) {
          ...
          nextPoint = Cartesian3.Cartesian3.fromArray(
            finalPositions,
            (start + j) * 3,
            nextPoint
          );
        ...
        previousPoint = Cartesian3.Cartesian3.subtract(
          Cartesian3.Cartesian3.fromArray(finalPositions, (start + j) * 3, previousPoint),
          outsidePoint,
          previousPoint
        );
        nextPoint = Cartesian3.Cartesian3.subtract(
          Cartesian3.Cartesian3.fromArray(finalPositions, start * 3, nextPoint),
          outsidePoint,
          nextPoint
        );
        left = Cartesian3.Cartesian3.normalize(
          Cartesian3.Cartesian3.negate(Cartesian3.Cartesian3.add(nextPoint, previousPoint, left), left),
          left
        );
        addNormals(attr, normal, left, undefined, back, vertexFormat);
        back -= 3;
      ...
      for (j = 0; j < leftEdge.length; j += 3) {
        rightNormal = ellipsoid.geodeticSurfaceNormal(
          Cartesian3.Cartesian3.fromArray(rightEdge, j, scratch1),
          scratch1
        );
        leftNormal = ellipsoid.geodeticSurfaceNormal(
          Cartesian3.Cartesian3.fromArray(leftEdge, length - j, scratch2),
          scratch2
        );
        normal = Cartesian3.Cartesian3.normalize(
          Cartesian3.Cartesian3.add(rightNormal, leftNormal, normal),
    ...
    normal = Cartesian3.Cartesian3.fromArray(
      computedNormals,
  ...
  function extrudedAttributes(attributes, vertexFormat) {
    ...
    if (vertexFormat.normal || vertexFormat.bitangent || vertexFormat.tangent) {
      ...
      for (i = 0; i < threeSize; i += 3) {
        ...
        bottomPosition = Cartesian3.Cartesian3.fromArray(
          positions,
          i + threeSize,
          bottomPosition
        );
        previousPosition = Cartesian3.Cartesian3.fromArray(
          positions,
          (i + 3) % threeSize,
          previousPosition
        );
        bottomPosition = Cartesian3.Cartesian3.subtract(
          bottomPosition,
          topPosition,
          bottomPosition
        );
        previousPosition = Cartesian3.Cartesian3.subtract(
          previousPosition,
          topPosition,
          previousPosition
        );
        normal = Cartesian3.Cartesian3.normalize(
          Cartesian3.Cartesian3.cross(bottomPosition, previousPosition, normal),
          normal
        );
        if (vertexFormat.normal) {
  ...
  function computeOffsetPoints(
    ...
    const direction = Cartesian3.Cartesian3.subtract(
      position2,
      position1,
      scratchCartesian1
    );
    ...
    const offsetDirection = Cartesian3.Cartesian3.cross(
      direction,
      normal,
      scratchCartesian1
    );
  ...
  const scratchCartesianOffset = new Cartesian3.Cartesian3();
  const scratchCartesianEnds = new Cartesian3.Cartesian3();
  const scratchCartographicMin = new Cartesian2.Cartographic();
  const scratchCartographicMax = new Cartesian2.Cartographic();
  ...
  function computeRectangle(positions, ellipsoid, width, cornerType, result) {
    ...
    const rectangle = defined.defined(result) ? result : new Cartesian2.Rectangle();
  ...
  function CorridorGeometry(options) {
    ...
    this.packedLength =
      1 +
      positions.length * Cartesian3.Cartesian3.packedLength +
  ...
  CorridorGeometry.computeRectangle = function (options, result) {
    ...
    const cornerType = defined.defaultValue(options.cornerType, PolylineVolumeGeometryLibrary.CornerType.ROUNDED);

...
define(['exports', './Transforms-e81b498a', './Cartesian3-5587e0cf', './Check-0f680516', './defined-a5305fd6', './Cartesian2-b941a975', './AttributeCompression-59630bdd', './ComponentDatatype-4ab1a86a', './Math-79d70b44', './Matrix2-81068516'], (function (exports, Transforms, Cartesian3, Check, defined, Cartesian2, AttributeCompression, ComponentDatatype, Math$1, Matrix2) { 'use strict';
  ...
  Object.defineProperties(EllipsoidalOccluder.prototype, {
    ...
    cameraPosition: {
      ...
      set: function (cameraPosition) {
        ...
        const vhMagnitudeSquared = Cartesian3.Cartesian3.magnitudeSquared(cv) - 1.0;
  ...
  const scratchCameraPositionInScaledSpaceShrunk = new Cartesian3.Cartesian3();
  ...
  const scratchEllipsoidShrunk = Cartesian2.Ellipsoid.clone(Cartesian2.Ellipsoid.UNIT_SPHERE);
  ...
  const scratchEllipsoidShrunkRadii = new Cartesian3.Cartesian3();
  ...
  function getPossiblyShrunkEllipsoid(ellipsoid, minimumHeight, result) {
    if (
      ...
      const ellipsoidShrunkRadii = Cartesian3.Cartesian3.fromElements(
  ...
  function isScaledSpacePointVisible(
    ...
    const vt = Cartesian3.Cartesian3.subtract(
      occludeeScaledSpacePosition,
      cv,
      scratchCartesian
    );
    const vtDotVc = -Cartesian3.Cartesian3.dot(vt, cv);
    ...
    const isOccluded =
      vhMagnitudeSquared < 0
        ...
        : vtDotVc > vhMagnitudeSquared &&
          (vtDotVc * vtDotVc) / Cartesian3.Cartesian3.magnitudeSquared(vt) >
            vhMagnitudeSquared;
    return !isOccluded;
  }

  const scaledSpaceScratch = new Cartesian3.Cartesian3();
  const directionScratch = new Cartesian3.Cartesian3();
  ...
  function computeMagnitude(ellipsoid, position, scaledSpaceDirectionToPoint) {
    ...
    let magnitudeSquared = Cartesian3.Cartesian3.magnitudeSquared(scaledSpacePosition);
    let magnitude = Math.sqrt(magnitudeSquared);
    const direction = Cartesian3.Cartesian3.divideByScalar(
      scaledSpacePosition,
      magnitude,
      directionScratch
    );
    ...
    const cosAlpha = Cartesian3.Cartesian3.dot(direction, scaledSpaceDirectionToPoint);
    const sinAlpha = Cartesian3.Cartesian3.magnitude(
      Cartesian3.Cartesian3.cross(direction, scaledSpaceDirectionToPoint, direction)
    );
  ...
  function magnitudeToPoint(
    ...
    return Cartesian3.Cartesian3.multiplyByScalar(
      scaledSpaceDirectionToPoint,
      resultMagnitude,
      result
    );
  ...
  const directionToPointScratch = new Cartesian3.Cartesian3();
  ...
  const scratchCartographic = new Cartesian3.Cartesian3();
  ...
  const cartesian3Scratch = new Cartesian3.Cartesian3();
  const cartesian3DimScratch = new Cartesian3.Cartesian3();
  const cartesian2Scratch = new Cartesian2.Cartesian2();
  ...
  function TerrainEncoding(
    ...
    if (
      defined.defined(axisAlignedBoundingBox) &&
      defined.defined(minimumHeight) &&
      defined.defined(maximumHeight) &&
      defined.defined(fromENU)
    ) {
      const minimum = axisAlignedBoundingBox.minimum;
      const maximum = axisAlignedBoundingBox.maximum;

      const dimensions = Cartesian3.Cartesian3.subtract(
        maximum,
        minimum,
        cartesian3DimScratch
      );
      const hDim = maximumHeight - minimumHeight;
      const maxDim = Math.max(Cartesian3.Cartesian3.maximumComponent(dimensions), hDim);
      ...
      const translation = Cartesian3.Cartesian3.negate(minimum, cartesian3Scratch);
      ...
      Matrix2.Matrix4.setTranslation(matrix, Cartesian3.Cartesian3.ZERO, matrix);
     ...
     * @type {Cartesian3}
     */
    this.center = Cartesian3.Cartesian3.clone(center);
     ...
     * @type {Boolean}
     */
    this.hasWebMercatorT = defined.defaultValue(hasWebMercatorT, false);

    /**
     * The terrain mesh contains geodetic surface normals, used for terrain exaggeration.
     * @type {Boolean}
     */
    this.hasGeodeticSurfaceNormals = defined.defaultValue(
      hasGeodeticSurfaceNormals,
      false
    );
     ...
     * @type {Number}
     */
    this.exaggeration = defined.defaultValue(exaggeration, 1.0);

    /**
     * The relative height from which terrain is exaggerated.
     */
    this.exaggerationRelativeHeight = defined.defaultValue(
      exaggerationRelativeHeight,
      0.0
    );
  ...
  const scratchGeodeticSurfaceNormal = new Cartesian3.Cartesian3();
...
//# sourceMappingURL=TerrainEncoding-578da000.js.map

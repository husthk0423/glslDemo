...
define(['exports', './Transforms-e81b498a', './Cartesian2-b941a975', './Cartesian3-5587e0cf', './ComponentDatatype-4ab1a86a', './defined-a5305fd6', './Check-0f680516', './GeometryAttribute-a065274b', './GeometryAttributes-ae79d5fa', './GeometryOffsetAttribute-102da468', './IndexDatatype-be8e0e62', './Math-79d70b44', './VertexFormat-26a1b05a'], (function (exports, Transforms, Cartesian2, Cartesian3, ComponentDatatype, defined, Check, GeometryAttribute, GeometryAttributes, GeometryOffsetAttribute, IndexDatatype, Math$1, VertexFormat) { 'use strict';
  ...
  const scratchNormalST = new Cartesian3.Cartesian3();
  const defaultRadii = new Cartesian3.Cartesian3(1.0, 1.0, 1.0);
  ...
  function EllipsoidGeometry(options) {
    ...
    const radii = defined.defaultValue(options.radii, defaultRadii);
    const innerRadii = defined.defaultValue(options.innerRadii, radii);
    const minimumClock = defined.defaultValue(options.minimumClock, 0.0);
    const maximumClock = defined.defaultValue(options.maximumClock, Math$1.CesiumMath.TWO_PI);
    const minimumCone = defined.defaultValue(options.minimumCone, 0.0);
    const maximumCone = defined.defaultValue(options.maximumCone, Math$1.CesiumMath.PI);
    const stackPartitions = Math.round(defined.defaultValue(options.stackPartitions, 64));
    const slicePartitions = Math.round(defined.defaultValue(options.slicePartitions, 64));
  ...
  EllipsoidGeometry.packedLength =
    2 * Cartesian3.Cartesian3.packedLength + VertexFormat.VertexFormat.packedLength + 7;
  ...
  const scratchRadii = new Cartesian3.Cartesian3();
  const scratchInnerRadii = new Cartesian3.Cartesian3();
  ...
  EllipsoidGeometry.unpack = function (array, startingIndex, result) {
    ...
    const radii = Cartesian3.Cartesian3.unpack(array, startingIndex, scratchRadii);
    startingIndex += Cartesian3.Cartesian3.packedLength;

    const innerRadii = Cartesian3.Cartesian3.unpack(array, startingIndex, scratchInnerRadii);
    startingIndex += Cartesian3.Cartesian3.packedLength;
  ...
  EllipsoidGeometry.createGeometry = function (ellipsoidGeometry) {
    ...
    let ellipsoid;
    const ellipsoidOuter = Cartesian2.Ellipsoid.fromCartesian3(radii);
    const ellipsoidInner = Cartesian2.Ellipsoid.fromCartesian3(innerRadii);
    ...
    if (
      ...
      for (i = 0; i < vertexCount; i++) {
        ...
        const position = Cartesian3.Cartesian3.fromArray(positions, i * 3, scratchPosition);
        ...
        if (vertexFormat.st) {
          const normalST = Cartesian2.Cartesian2.negate(normal, scratchNormalST);
          st[stIndex++] =
        ...
        if (vertexFormat.tangent || vertexFormat.bitangent) {
          ...
          if (vertexFormat.bitangent) {
            const bitangent = Cartesian3.Cartesian3.cross(normal, tangent, scratchBitangent);
...
//# sourceMappingURL=EllipsoidGeometry-bdc49dd8.js.map

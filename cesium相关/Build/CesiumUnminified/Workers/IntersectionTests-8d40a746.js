...
define(['exports', './Cartesian3-5587e0cf', './Cartesian2-b941a975', './defined-a5305fd6', './Check-0f680516', './Transforms-e81b498a', './Math-79d70b44', './Matrix2-81068516'], (function (exports, Cartesian3, Cartesian2, defined, Check, Transforms, Math$1, Matrix2) { 'use strict';
  ...
  function Ray(origin, direction) {
    direction = Cartesian3.Cartesian3.clone(defined.defaultValue(direction, Cartesian3.Cartesian3.ZERO));
     ...
     */
    this.origin = Cartesian3.Cartesian3.clone(defined.defaultValue(origin, Cartesian3.Cartesian3.ZERO));
  ...
  IntersectionTests.rayPlane = function (ray, plane, result) {
    ...
    const denominator = Cartesian3.Cartesian3.dot(normal, direction);
    ...
    const t = (-plane.distance - Cartesian3.Cartesian3.dot(normal, origin)) / denominator;
  ...
  const scratchEdge0 = new Cartesian3.Cartesian3();
  const scratchEdge1 = new Cartesian3.Cartesian3();
  const scratchPVec = new Cartesian3.Cartesian3();
  const scratchTVec = new Cartesian3.Cartesian3();
  const scratchQVec = new Cartesian3.Cartesian3();
  ...
  IntersectionTests.rayTriangleParametric = function (
    ...
    cullBackFaces = defined.defaultValue(cullBackFaces, false);

    const origin = ray.origin;
    const direction = ray.direction;

    const edge0 = Cartesian3.Cartesian3.subtract(p1, p0, scratchEdge0);
    const edge1 = Cartesian3.Cartesian3.subtract(p2, p0, scratchEdge1);

    const p = Cartesian3.Cartesian3.cross(direction, edge1, scratchPVec);
    const det = Cartesian3.Cartesian3.dot(edge0, p);
  ...
  IntersectionTests.lineSegmentTriangle = function (
    ...
    if (!defined.defined(t) || t < 0.0 || t > Cartesian3.Cartesian3.distance(v0, v1)) {
      return undefined;
    }
  ...
  function raySphere(ray, sphere, result) {
    ...
    const diff = Cartesian3.Cartesian3.subtract(origin, center, scratchPVec);

    const a = Cartesian3.Cartesian3.dot(direction, direction);
    const b = 2.0 * Cartesian3.Cartesian3.dot(direction, diff);
    const c = Cartesian3.Cartesian3.magnitudeSquared(diff) - radiusSquared;
  ...
  IntersectionTests.lineSegmentSphere = function (p0, p1, sphere, result) {
    ...
    const direction = Cartesian3.Cartesian3.subtract(p1, p0, ray.direction);

    const maxT = Cartesian3.Cartesian3.magnitude(direction);
    Cartesian3.Cartesian3.normalize(direction, direction);
  ...
  const scratchQ = new Cartesian3.Cartesian3();
  const scratchW = new Cartesian3.Cartesian3();
  ...
  IntersectionTests.rayEllipsoid = function (ray, ellipsoid) {
    ...
    const q = Cartesian3.Cartesian3.multiplyComponents(inverseRadii, ray.origin, scratchQ);
    const w = Cartesian3.Cartesian3.multiplyComponents(
      inverseRadii,
      ray.direction,
      scratchW
    );

    const q2 = Cartesian3.Cartesian3.magnitudeSquared(q);
    const qw = Cartesian3.Cartesian3.dot(q, w);
  ...
  const firstAxisScratch = new Cartesian3.Cartesian3();
  const secondAxisScratch = new Cartesian3.Cartesian3();
  const thirdAxisScratch = new Cartesian3.Cartesian3();
  const referenceScratch = new Cartesian3.Cartesian3();
  const bCart = new Cartesian3.Cartesian3();
  ...
  const sScratch = new Cartesian3.Cartesian3();
  const closestScratch = new Cartesian3.Cartesian3();
  const surfPointScratch = new Cartesian2.Cartographic();
  ...
  IntersectionTests.grazingAltitudeLocation = function (ray, ellipsoid) {
    ...
    if (!Cartesian3.Cartesian3.equals(position, Cartesian3.Cartesian3.ZERO)) {
      ...
      if (Cartesian3.Cartesian3.dot(direction, normal) >= 0.0) {
        // The location provided is the closest point in altitude
        return position;
      }
    }

    const intersects = defined.defined(this.rayEllipsoid(ray, ellipsoid));
    ...
    const firstAxis = Cartesian3.Cartesian3.normalize(f, f);
    const reference = Cartesian3.Cartesian3.mostOrthogonalAxis(f, referenceScratch);
    const secondAxis = Cartesian3.Cartesian3.normalize(
      Cartesian3.Cartesian3.cross(reference, firstAxis, secondAxisScratch),
      secondAxisScratch
    );
    const thirdAxis = Cartesian3.Cartesian3.normalize(
      Cartesian3.Cartesian3.cross(firstAxis, secondAxis, thirdAxisScratch),
      thirdAxisScratch
    );
    const B = bScratch;
    B[0] = firstAxis.x;
    ...
    const solutions = quadraticVectorExpression(
      A,
      Cartesian3.Cartesian3.negate(b, firstAxisScratch),
      0.0,
      0.0,
      1.0
    );
    ...
    if (length > 0) {
      let closest = Cartesian3.Cartesian3.clone(Cartesian3.Cartesian3.ZERO, closestScratch);
      ...
      for (let i = 0; i < length; ++i) {
        ...
        const v = Cartesian3.Cartesian3.normalize(
          Cartesian3.Cartesian3.subtract(s, position, referenceScratch),
          referenceScratch
        );
        const dotProduct = Cartesian3.Cartesian3.dot(v, direction);
      ...
      altitude =
        Cartesian3.Cartesian3.magnitude(
          Cartesian3.Cartesian3.subtract(closest, position, referenceScratch)
  ...
  const lineSegmentPlaneDifference = new Cartesian3.Cartesian3();
  ...
  IntersectionTests.lineSegmentPlane = function (
    ...
    const difference = Cartesian3.Cartesian3.subtract(
      endPoint1,
      endPoint0,
      lineSegmentPlaneDifference
    );
    const normal = plane.normal;
    const nDotDiff = Cartesian3.Cartesian3.dot(normal, difference);
    ...
    const nDotP0 = Cartesian3.Cartesian3.dot(normal, endPoint0);
  ...
  IntersectionTests.trianglePlaneIntersection = function (p0, p1, p2, plane) {
    //>>includeStart('debug', pragmas.debug);
    if (!defined.defined(p0) || !defined.defined(p1) || !defined.defined(p2) || !defined.defined(plane)) {
    ...
    const p0Behind = Cartesian3.Cartesian3.dot(planeNormal, p0) + planeD < 0.0;
    const p1Behind = Cartesian3.Cartesian3.dot(planeNormal, p1) + planeD < 0.0;
    const p2Behind = Cartesian3.Cartesian3.dot(planeNormal, p2) + planeD < 0.0;
...
//# sourceMappingURL=IntersectionTests-8d40a746.js.map

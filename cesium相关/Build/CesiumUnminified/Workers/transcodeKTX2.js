...
define(['./defined-a5305fd6', './Check-0f680516', './PixelFormat-3c3c79f0', './RuntimeError-8d8b6ef6', './createTaskProcessorWorker', './WebGLConstants-d81b330d'], (function (defined, Check, PixelFormat, RuntimeError, createTaskProcessorWorker, WebGLConstants) { 'use strict';
  ...
  function parseUncompressed(header, result) {
    ...
    if (header.vkFormat === VulkanConstants$1.VK_FORMAT_R8G8B8A8_UNORM) {
      datatype = PixelFormat.PixelDatatype.UNSIGNED_BYTE;
    } else if (
      header.vkFormat === VulkanConstants$1.VK_FORMAT_R16G16B16A16_SFLOAT
    ) {
      datatype = PixelFormat.PixelDatatype.HALF_FLOAT;
    } else if (
      header.vkFormat === VulkanConstants$1.VK_FORMAT_R32G32B32A32_SFLOAT
    ) {
      datatype = PixelFormat.PixelDatatype.FLOAT;
    }
    ...
    for (let i = 0; i < header.levels.length; ++i) {
      ...
      const faceLength =
        width * height * PixelFormat.PixelFormat.componentsLength(internalFormat);
      ...
      for (let j = 0; j < header.faceCount; ++j) {
        ...
        let faceView;
        if (!defined.defined(datatype) || PixelFormat.PixelDatatype.sizeInBytes(datatype) === 1) {
          faceView = new Uint8Array(
            levelBuffer.buffer,
            faceByteOffset,
            faceLength
          );
        } else if (PixelFormat.PixelDatatype.sizeInBytes(datatype) === 2) {
  ...
  function transcodeCompressed(
    ...
    if (dfd.colorModel === colorModelETC1S) {
      if (supportedTargetFormats.etc) {
        ...
        internalFormat = PixelFormat.PixelFormat.RGB_ETC1;
        transcoderFormat = BasisFormat.cTFETC1_RGB;
      } else if (supportedTargetFormats.s3tc) {
        internalFormat = hasAlpha ? PixelFormat.PixelFormat.RGBA_DXT5 : PixelFormat.PixelFormat.RGB_DXT1;
        transcoderFormat = hasAlpha
        ...
        internalFormat = PixelFormat.PixelFormat.RGBA_ASTC;
        transcoderFormat = BasisFormat.cTFASTC_4x4_RGBA;
      } else if (supportedTargetFormats.bc7) {
        internalFormat = PixelFormat.PixelFormat.RGBA_BC7;
    ...
    if (defined.defined(wasmConfig)) {
      ...
      return require([wasmConfig.modulePath], function (mscBasisTranscoder) {
        if (defined.defined(wasmConfig.wasmBinaryFile)) {
          if (!defined.defined(mscBasisTranscoder)) {

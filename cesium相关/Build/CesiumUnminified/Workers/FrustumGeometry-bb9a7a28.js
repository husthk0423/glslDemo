...
define(['exports', './Transforms-e81b498a', './Cartesian3-5587e0cf', './Matrix2-81068516', './Check-0f680516', './ComponentDatatype-4ab1a86a', './defined-a5305fd6', './GeometryAttribute-a065274b', './GeometryAttributes-ae79d5fa', './Math-79d70b44', './Plane-20e816c1', './VertexFormat-26a1b05a'], (function (exports, Transforms, Cartesian3, Matrix2, Check, ComponentDatatype, defined, GeometryAttribute, GeometryAttributes, Math$1, Plane, VertexFormat) { 'use strict';
  ...
  function CullingVolume(planes) {
     ...
     * @default []
     */
    this.planes = defined.defaultValue(planes, []);
  }

  const faces = [new Cartesian3.Cartesian3(), new Cartesian3.Cartesian3(), new Cartesian3.Cartesian3()];
  ...
  const scratchPlaneCenter = new Cartesian3.Cartesian3();
  const scratchPlaneNormal = new Cartesian3.Cartesian3();
  const scratchPlane = new Plane.Plane(new Cartesian3.Cartesian3(1.0, 0.0, 0.0), 0.0);
  ...
  CullingVolume.fromBoundingSphere = function (boundingSphere, result) {
    ...
    for (let i = 0; i < length; ++i) {
      ...
      plane1.w = -Cartesian3.Cartesian3.dot(
        Cartesian3.Cartesian3.negate(faceNormal, scratchPlaneNormal),
        scratchPlaneCenter
      );
  ...
  function OrthographicOffCenterFrustum(options) {
     ...
     * @default 1.0
     */
    this.near = defined.defaultValue(options.near, 1.0);
     ...
     * @default 500000000.0;
     */
    this.far = defined.defaultValue(options.far, 500000000.0);
    this._far = this.far;
  ...
  function update$3(frustum) {
    //>>includeStart('debug', pragmas.debug);
    if (
      !defined.defined(frustum.right) ||
      !defined.defined(frustum.left) ||
      !defined.defined(frustum.top) ||
      !defined.defined(frustum.bottom) ||
      !defined.defined(frustum.near) ||
      !defined.defined(frustum.far)
    ) {
  ...
  const getPlanesRight$1 = new Cartesian3.Cartesian3();
  const getPlanesNearCenter$1 = new Cartesian3.Cartesian3();
  const getPlanesPoint = new Cartesian3.Cartesian3();
  const negateScratch = new Cartesian3.Cartesian3();
  ...
  OrthographicOffCenterFrustum.prototype.computeCullingVolume = function (
    ...
    const right = Cartesian3.Cartesian3.cross(direction, up, getPlanesRight$1);
  ...
  function update$2(frustum) {
    //>>includeStart('debug', pragmas.debug);
    if (
      !defined.defined(frustum.width) ||
      !defined.defined(frustum.aspectRatio) ||
      !defined.defined(frustum.near) ||
      !defined.defined(frustum.far)
    ) {
      throw new Check.DeveloperError(
  ...
  const getPlanesRight = new Cartesian3.Cartesian3();
  const getPlanesNearCenter = new Cartesian3.Cartesian3();
  const getPlanesFarCenter = new Cartesian3.Cartesian3();
  const getPlanesNormal = new Cartesian3.Cartesian3();
  /**
   * Creates a culling volume for this frustum.
   *
   * @param {Cartesian3} position The eye position.
  ...
  PerspectiveOffCenterFrustum.prototype.computeCullingVolume = function (
    ...
    const right = Cartesian3.Cartesian3.cross(direction, up, getPlanesRight);
  ...
  function PerspectiveFrustum(options) {
     ...
     * @default 0.0
     */
    this.xOffset = defined.defaultValue(options.xOffset, 0.0);
     ...
     * @default 0.0
     */
    this.yOffset = defined.defaultValue(options.yOffset, 0.0);
  ...
  function FrustumGeometry(options) {
    ...
    const drawNearPlane = defined.defaultValue(options._drawNearPlane, true);
  ...
  const scratchPackorigin = new Cartesian3.Cartesian3();
  ...
  FrustumGeometry.unpack = function (array, startingIndex, result) {
    ...
    const origin = Cartesian3.Cartesian3.unpack(array, startingIndex, scratchPackorigin);
    startingIndex += Cartesian3.Cartesian3.packedLength;
  ...
  const scratchXDirection = new Cartesian3.Cartesian3();
  const scratchYDirection = new Cartesian3.Cartesian3();
  const scratchZDirection = new Cartesian3.Cartesian3();
  const scratchNegativeX = new Cartesian3.Cartesian3();
  const scratchNegativeY = new Cartesian3.Cartesian3();
  const scratchNegativeZ = new Cartesian3.Cartesian3();
  ...
  FrustumGeometry._computeNearFarPlanes = function (
    ...
    let x = defined.defaultValue(xDirection, scratchXDirection);
    let y = defined.defaultValue(yDirection, scratchYDirection);
    let z = defined.defaultValue(zDirection, scratchZDirection);
    ...
    for (let i = 0; i < 2; ++i) {
      for (let j = 0; j < 4; ++j) {
        ...
        if (!defined.defined(inverseViewProjection)) {
          ...
          const fac = Cartesian3.Cartesian3.dot(z, corner);
  ...
  FrustumGeometry.createGeometry = function (frustumGeometry) {
    ...
    if (
      defined.defined(vertexFormat.normal) ||
      defined.defined(vertexFormat.tangent) ||
      defined.defined(vertexFormat.bitangent) ||
      defined.defined(vertexFormat.st)
    ) {
      const normals = defined.defined(vertexFormat.normal)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const tangents = defined.defined(vertexFormat.tangent)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const bitangents = defined.defined(vertexFormat.bitangent)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const st = defined.defined(vertexFormat.st)
      ...
      const negativeX = Cartesian3.Cartesian3.negate(x, scratchNegativeX);
      const negativeY = Cartesian3.Cartesian3.negate(y, scratchNegativeY);
      const negativeZ = Cartesian3.Cartesian3.negate(z, scratchNegativeZ);
...
//# sourceMappingURL=FrustumGeometry-bb9a7a28.js.map
